...
## 1. Introdução
<p style="text-align: justify;"> O Léxico é uma técnica que procura descrever os símbolos de uma linguagem. Sendo o principal objetivo dos Engenheiros de Requisitos, buscar frases e símbolos do domínio da aplicação. Cada símbolo é descrito com uma noção e um impacto, sendo a noção o símbolo e o impacto a descrição do efeito do símbolo na aplicação ou do efeito de algo na aplicação sobre o símbolo.
</p>
